...
from decomposable import DecomposableNLIModel, attention_softmax3d, mask_3d
import numpy as np
...
class MultiFeedForwardClassifier(DecomposableNLIModel):
    ...
    def __init__(self, num_units, num_classes, vocab_size, embedding_size,
                 ...
                 use_intra_attention=False, distance_biases=10, train_embed=False, use_rnn=False, use_pe=False,
                 ranking=False, bs=32, use_bias=True, use_masked_attn=False):
        """
    ...
    def _transformation_input(self, inputs, size, reuse_weights=False, seq_lens=None):
        ...
        transformed = super(MultiFeedForwardClassifier, self).\
            _transformation_input(inputs, size, reuse_weights)
    ...
    @classmethod
    def _init_from_load(cls, params, training, ranking=False, optimizer='adagrad', batch_size=32):
    ...
    def compute_intra_attention(self, sentence, sentence_size, reuse_weights=False, use_bias=True):
        """
        ...
        with tf.variable_scope('intra-attention') as scope:
            ...
            raw_attentions += bias
            masked = mask_3d(raw_attentions, sentence_size, -np.inf)
            attentions = attention_softmax3d(masked)
