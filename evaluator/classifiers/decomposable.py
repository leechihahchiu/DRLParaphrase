...
def create_msk(bs, sent1, sent2, size1s, size2s):
    max_s1 = int(np.max(size1s))
    max_s2 = int(np.max(size2s))
...
def clip_sentence(sentence, sizes):
    ...
    # NOTE: this is a terrible hardcode
    max_batch_size = tf.clip_by_value(max_batch_size, 1, 21)
...
class DecomposableNLIModel(object):
    ...
    def __init__(self, num_units, num_classes, vocab_size, embedding_size,
                 ...
                 train_embed=False, use_pe=False, ranking=False, bs=32):
        """
        ...
        self.msk1 = tf.placeholder(tf.int32, [], 'msk')
        self.msk2 = tf.placeholder(tf.int32, [], 'msk2')
    ...
    def _pe(self, inputs, sizes):
        num_chanels = self.embedding_size
        idx = range(21)
        pe = np.zeros(num_chanels*21)
        pos = np.array(list(range(1, 21))*num_chanels)
        dmodel = []
        for x in range(num_chanels):
            dmodel += [x]*21

        dmodel = np.array(dmodel)
        pe[idx[0::2]] = np.cos(( pos[idx[0::2]] / 1e4 ) ** ( 2*dmodel[idx[0::2]] / num_chanels ))
        pe[idx[1::2]] = np.cos(( pos[idx[1::2]] / 1e4 ) ** ( 2*dmodel[idx[1::2]] / num_chanels ))
        pe = np.expand_dims(
            np.reshape(pe, [21, num_chanels]),
            axis=0
            )
        max_batch_size = tf.reduce_max(sizes)
        pe = tf.slice(pe, [0, 0, 0], [-1, max_batch_size, -1])

        return tf.cast(pe, tf.float32) + inputs
    ...
    def _transformation_input(self, inputs, reuse_weights=False):
        ...
        if self.project_input:
            ...
            projected = self._pe(projected, sizes)
        else:
            projected = inputs
    ...
    def _create_batch_feed(self, batch_data, learning_rate, dropout_keep,
        ...
        """
        sz1 = [x if x <= 21 else 21 for x in list(batch_data.sizes1)]
        sz2 = [x if x <= 21 else 21 for x in list(batch_data.sizes2)]
        #msk1 = np.zeros((len(sz1), 21))
        #msk2 = np.zeros((len(sz2), 21))
        #for i, (s1, s2) in enumerate(zip(batch_data.sentences1, batch_data.sentences2)):
        #    msk1[i, :] = [x in s2[:21] for x in list(s1[:21])]
    ...
    def create_outer_batch_feed(
        self, sent1, sent2, sz1, sz2, labels, lr, dropout_keep, l2, clip_value):
        bs = len(sz1)
        nulls = np.ones((bs, 1)) * 2
        sent1 = np.concatenate(
            (nulls, sent1),
            axis=1)
    ...
    def eval(self, session, src_sents, tg_sents, sz1, sz2, debug=False):
        ...
        nulls = np.ones((bs, 1)) * 2
        src_sents = np.concatenate(
            (nulls, src_sents),
             axis=1)
        tg_sents = np.concatenate(
            (nulls, tg_sents),
            axis=1)
        sz1 = sz1 + 1
        sz2 = sz2 + 1
        msk1, msk2 = create_msk(bs, src_sents, tg_sents, sz1, sz2)
        feeds = {
            self.sentence1: src_sents,
            self.sentence2: tg_sents,
            self.sentence1_size: sz1,
            self.sentence2_size: sz2,
            self.msk1: msk1,
            self.msk2: msk2,
        ...
        dist = logits / np.expand_dims(np.sum(logits, axis=1), 1)
        if debug:
            tmp = session.run(self.tmp, feeds)
            return scores, tmp
        else:
            return scores
    ...
    def train(self, session, train_dataset, valid_dataset, save_dir,
        ...
        for i in range(num_epochs):
            ...
            while batch_index < train_dataset.num_items:
                ...
                _, loss, tmp = session.run(ops, feed_dict=feeds)
